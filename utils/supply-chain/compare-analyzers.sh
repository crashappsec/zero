...
check_prerequisites() {
    ...
    if [[ ! -x "$SCRIPT_DIR/supply-chain-analyzer.sh" ]]; then
        echo -e "${RED}Error: supply-chain-analyzer.sh not found or not executable${NC}"
        exit 1
    fi

    # Check if Claude analyzer exists
    if [[ ! -x "$SCRIPT_DIR/supply-chain-analyzer-claude.sh" ]]; then
        echo -e "${RED}Error: supply-chain-analyzer-claude.sh not found or not executable${NC}"
        exit 1
    fi
...
run_basic_analyzer() {
    ...
    local cmd="$SCRIPT_DIR/supply-chain-analyzer.sh --format json --output $output_file"
...
run_claude_analyzer() {
    ...
    local cmd="ANTHROPIC_API_KEY=$ANTHROPIC_API_KEY $SCRIPT_DIR/supply-chain-analyzer-claude.sh"
