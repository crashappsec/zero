...
#
# Global library for interacting with the deps.dev API.
...
SUPPLY_CHAIN_DIR="$(dirname "$DEPS_DEV_LIB_DIR")"
UTILS_ROOT="$(dirname "$SUPPLY_CHAIN_DIR")"

# Load configuration if available
CONFIG="{}"
if [ -f "$UTILS_ROOT/lib/config-loader.sh" ]; then
    source "$UTILS_ROOT/lib/config-loader.sh"
    CONFIG=$(load_config "supply-chain" 2>/dev/null || echo "{}")
fi

# Configuration with defaults
DEPS_DEV_BASE_URL="${DEPS_DEV_BASE_URL:-$(echo "$CONFIG" | jq -r '.api.deps_dev_base_url // "https://api.deps.dev/v3alpha"')}"
API_TIMEOUT="${DEPS_DEV_API_TIMEOUT:-$(echo "$CONFIG" | jq -r '.api.timeout // 30')}"
RETRY_ATTEMPTS="${DEPS_DEV_RETRY_ATTEMPTS:-$(echo "$CONFIG" | jq -r '.api.retry_attempts // 3')}"
RATE_LIMIT_DELAY="${DEPS_DEV_RATE_LIMIT_DELAY:-$(echo "$CONFIG" | jq -r '.api.rate_limit_delay // 1')}"

# Ensure defaults if jq returned null or empty
[[ -z "$DEPS_DEV_BASE_URL" || "$DEPS_DEV_BASE_URL" == "null" ]] && DEPS_DEV_BASE_URL="https://api.deps.dev/v3alpha"
[[ -z "$API_TIMEOUT" || "$API_TIMEOUT" == "null" ]] && API_TIMEOUT=30
[[ -z "$RETRY_ATTEMPTS" || "$RETRY_ATTEMPTS" == "null" ]] && RETRY_ATTEMPTS=3
[[ -z "$RATE_LIMIT_DELAY" || "$RATE_LIMIT_DELAY" == "null" ]] && RATE_LIMIT_DELAY=1

# Cache configuration
CACHE_ENABLED="${DEPS_DEV_CACHE_ENABLED:-$(echo "$CONFIG" | jq -r '.cache.enabled // true')}"
CACHE_TTL_HOURS="${DEPS_DEV_CACHE_TTL_HOURS:-$(echo "$CONFIG" | jq -r '.cache.ttl_hours // 24')}"
CACHE_DIR="${DEPS_DEV_CACHE_DIR:-$(echo "$CONFIG" | jq -r '.cache.cache_dir // "/tmp/deps-dev-cache"')}"
