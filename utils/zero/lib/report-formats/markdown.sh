...
format_report_output() {
    ...
    case "$report_type" in
        ...
        ai-adoption)
            ...
            ;;
        malcontent)
            format_malcontent_markdown "$json_data" "$target_id"
            ;;
        *)
            format_summary_markdown "$json_data" "$target_id"
            ;;
...
# Format malcontent report for markdown (Scout agent aligned)
format_malcontent_markdown() {
    local json="$1"
    local target_id="$2"
    ...
    if [[ "$is_org" == "true" ]]; then
        format_org_malcontent_markdown "$json"
    else
        format_project_malcontent_markdown "$json"
    fi
}

# Format project malcontent report for markdown
format_project_malcontent_markdown() {
    local json="$1"
    ...
    local profile=$(echo "$json" | jq -r '.project.profile')
    local completed_at=$(echo "$json" | jq -r '.project.completed_at')
    local risk_level=$(echo "$json" | jq -r '.risk.level // "low"')

    # Summary metrics
    local total_files=$(echo "$json" | jq -r '.summary.total_files_scanned // 0')
    local total_rules=$(echo "$json" | jq -r '.summary.total_rules_matched // 0')
    local total_packages=$(echo "$json" | jq -r '.summary.total_packages // 0')
    local critical=$(echo "$json" | jq -r '.summary.by_severity.critical // 0')
    local high=$(echo "$json" | jq -r '.summary.by_severity.high // 0')
    local medium=$(echo "$json" | jq -r '.summary.by_severity.medium // 0')
    local low=$(echo "$json" | jq -r '.summary.by_severity.low // 0')

    local risk_color=$(get_badge_color "$risk_level")
    local risk_upper=$(echo "$risk_level" | tr '[:lower:]' '[:upper:]')
    local risk_emoji=$(get_risk_emoji "$risk_level")
    local scanned_at=$(format_timestamp "$completed_at")

    # Compute badges
    local risk_badge=$(badge "Risk" "$risk_upper" "$risk_color")
    local crit_badge=$(badge "Critical" "$critical" "$(if [[ $critical -gt 0 ]]; then echo "red"; else echo "green"; fi)")
    local high_badge=$(badge "High" "$high" "$(if [[ $high -gt 0 ]]; then echo "orange"; else echo "green"; fi)")
    local files_badge=$(badge "Files%20Scanned" "$total_files" "blue")

    # Risk description
    local risk_desc=""
    case "$risk_level" in
        critical) risk_desc="**Immediate action required.** Potential malicious code detected." ;;
        high) risk_desc="**Review urgently.** Suspicious behaviors detected that require investigation." ;;
        medium) risk_desc="**Investigate when possible.** Unusual patterns found that may warrant attention." ;;
        low) risk_desc="**No significant threats detected.** Continue monitoring." ;;
    esac

    # Output markdown
    cat << EOF
# Scout: Supply Chain Security Report

**Repository:** \`$project_id\`

$risk_badge $crit_badge $high_badge $files_badge
...
| **Scanned** | $scanned_at |
| **Profile** | $profile |
| **Analyzer** | Chainguard Malcontent (14,500+ YARA rules) |

---

## Risk Assessment

### Overall Risk: $risk_emoji **$risk_upper**
...
|----------|-------|-------------|
| 🔴 Critical | $critical | Potential malicious code |
| 🟠 High | $high | Suspicious behaviors |
| 🟡 Medium | $medium | Unusual patterns |
| ⚪ Low | $low | Informational |

**Total Rules Triggered:** $total_rules
EOF

    if [[ "$total_packages" -gt 0 ]]; then
        echo "**Packages in SBOM:** $total_packages"
    fi

    # Critical findings
    local crit_count=$(echo "$json" | jq -r '.findings.critical | length // 0')
...
> **These files may contain malicious code and should be reviewed immediately.**

EOF
        # Process all critical findings with relative paths and behavior tables
        echo "$json" | jq -r '.findings.critical[] |
            (.path | split("/repo/") | if length > 1 then .[1] else . end) as $relpath |
            "### `\($relpath)`\n\n| Behavior | Evidence |\n|----------|----------|\n" +
            ([.behaviors[] | "| \(.description // .id) | `\(.matches // [] | .[0:2] | join("`, `"))` |"] | join("\n")) +
            "\n"' 2>/dev/null || true
    fi

    # High findings
    local high_count=$(echo "$json" | jq -r '.findings.high | length // 0')
...
> **These files exhibit suspicious behaviors that should be investigated.**

EOF
        # Process first 15 high-risk findings with relative paths and code evidence
        echo "$json" | jq -r '.findings.high[:15][] |
            # Extract relative path (after /repo/)
            (.path | split("/repo/") | if length > 1 then .[1] else . end) as $relpath |
            "### `\($relpath)`\n\n" +
            ([.behaviors[:3][] |
                "| \(.description // .id) | `\(.matches // [] | .[0:2] | join("`, `"))` |"
            ] |
            "| Behavior | Evidence |\n|----------|----------|\n" + join("\n")) +
            "\n"' 2>/dev/null || true

        if [[ "$high_count" -gt 15 ]]; then
            echo "*... and $((high_count - 15)) more high-risk files*"
            echo
        fi
    fi

    # Medium findings (summary table)
    local med_count=$(echo "$json" | jq -r '.findings.medium | length // 0')
    if [[ "$med_count" -gt 0 ]]; then
        cat << EOF

---

## 🟡 Medium-Risk Findings (Top 10 of $med_count)

| File | Behavior | Evidence |
|------|----------|----------|
EOF
        echo "$json" | jq -r '.findings.medium[:10][] |
            (.path | split("/repo/") | if length > 1 then .[1] else . end) as $relpath |
            "| `\($relpath)` | \(.behaviors[0].description // .behaviors[0].id // "suspicious pattern") | `\(.behaviors[0].matches // [] | .[0] // "")` |"' 2>/dev/null || true
        echo
    fi
...
EOF
    ...
    if [[ "$critical" -gt 0 ]]; then
        cat << EOF
1. 🔴 **IMMEDIATE:** Review critical files for potential backdoors or data exfiltration
2. 🔴 **Quarantine** affected code paths until investigation is complete
3. 🔴 **Audit git history** for unauthorized commits or suspicious contributors
4. Consider engaging security incident response team
EOF
    elif [[ "$high" -gt 0 ]]; then
        cat << EOF
1. 🟠 Review high-risk files to determine if behaviors are intentional or malicious
2. 🟠 Audit external network access patterns in flagged code
3. 🟠 Verify any encoded/obfuscated code sections have legitimate purposes
4. Schedule security review in next sprint
EOF
    else
        cat << EOF
1. ✅ No critical or high-risk findings detected
2. Review medium-risk items during regular security reviews
3. Continue periodic malcontent scans as part of CI/CD pipeline
EOF
    fi
...
This report was generated by the **Scout** supply chain security agent using
[Chainguard Malcontent](https://github.com/chainguard-dev/malcontent), which
analyzes code for suspicious behaviors using 14,500+ YARA rules.

**Risk Levels:**
- **Critical:** Code patterns matching known malware signatures
- **High:** Suspicious behaviors like data exfiltration, credential access
- **Medium:** Unusual patterns requiring context to determine intent
- **Low:** Informational findings with minimal security impact

</details>

---

*Scout Agent • Phantom Supply Chain Security • v${REPORT_VERSION:-1.0.0}*
*$(date '+%Y-%m-%d %H:%M:%S %Z')*
EOF
}

# Format org malcontent report for markdown
format_org_malcontent_markdown() {
    local json="$1"
    ...
    cat << EOF
# Scout: Organization Security Report

**Organization:** \`$org\`

$(badge "Risk" "$risk_upper" "$risk_color") $(badge "Repositories" "$project_count" "blue") $(badge "Critical" "$critical" "$(if [[ $critical -gt 0 ]]; then echo "red"; else echo "green"; fi)") $(badge "High" "$high" "$(if [[ $high -gt 0 ]]; then echo "orange"; else echo "green"; fi)")
...
| Metric | Value |
|--------|-------|
| **Repositories Scanned** | $project_count |
| **Files Analyzed** | $total_files |
| **Overall Risk** | $risk_upper |
...
EOF
    ...
    local repos_with_issues=$(echo "$json" | jq -r '.projects.with_issues // [] | .[]' 2>/dev/null)
    if [[ -n "$repos_with_issues" ]]; then
        cat << EOF
...
The following repositories have critical or high-risk findings:

EOF
        echo "$repos_with_issues" | while read -r repo; do
            echo "- \`$repo\`"
        done
        echo
    fi
...
export -f format_malcontent_markdown
export -f format_project_malcontent_markdown
export -f format_org_malcontent_markdown
