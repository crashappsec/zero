...
format_report_output() {
    ...
    case "$report_type" in
        ...
        ai-adoption)
            ...
            ;;
        malcontent)
            format_malcontent_terminal "$json_data" "$target_id"
            ;;
        *)
            format_summary_terminal "$json_data" "$target_id"
            ;;
...
# Format malcontent report for terminal (Scout agent aligned)
format_malcontent_terminal() {
    local json="$1"
    local target_id="$2"
    ...
    if [[ "$is_org" == "true" ]]; then
        format_org_malcontent_terminal "$json"
    else
        format_project_malcontent_terminal "$json"
    fi
}

# Format project malcontent report for terminal
format_project_malcontent_terminal() {
    local json="$1"
    ...
    printf '%*s' 66 '' | tr ' ' "$BOX_H"
    printf "${BOX_TR}${NC}\n"
    printf "${BOLD}${BOX_V}  SCOUT: SUPPLY CHAIN SECURITY REPORT%*s${BOX_V}${NC}\n" 28 ''
    printf "${BOLD}${BOX_BL}"
    printf '%*s' 66 '' | tr ' ' "$BOX_H"
    printf "${BOX_BR}${NC}\n"
    echo
    ...
    printf "  ${BOLD}Scanned:${NC}     %s ${DIM}(%s)${NC}\n" "$(format_timestamp "$completed_at")" "$(relative_time "$completed_at")"
    printf "  ${BOLD}Profile:${NC}     %s\n" "$profile"
    printf "  ${BOLD}Analyzer:${NC}    Chainguard Malcontent (14,500+ YARA rules)\n"
    ...
    # Risk assessment
    printf "  ${BOLD}RISK ASSESSMENT${NC}\n"
    printf "  %s\n" "$(printf '%*s' 64 '' | tr ' ' '─')"

    printf "  %-20s │  " "Overall Risk"
    case "$risk_level" in
        critical) printf "${RED}CRITICAL${NC} - Immediate action required\n" ;;
        high) printf "${YELLOW}HIGH${NC} - Review urgently\n" ;;
        medium) printf "${CYAN}MEDIUM${NC} - Investigate when possible\n" ;;
        low) printf "${GREEN}LOW${NC} - No significant threats detected\n" ;;
        *) printf "%s\n" "$risk_level" ;;
    esac

    printf "  %-20s │  %s files analyzed\n" "Coverage" "$total_files"
    printf "  %-20s │  %s rules triggered\n" "Detections" "$total_rules"
    if [[ "$total_packages" -gt 0 ]]; then
        printf "  %-20s │  %s packages in SBOM\n" "Dependencies" "$total_packages"
    fi
    ...
    printf "  ${BOLD}FINDINGS BY SEVERITY${NC}\n"
    printf "  %s\n" "$(printf '%*s' 64 '' | tr ' ' '─')"

    printf "  %-20s │  " "Critical"
    if [[ "$critical" -gt 0 ]]; then
        printf "${RED}%s files${NC} - Potential malicious code\n" "$critical"
    else
        printf "${GREEN}0${NC}\n"
    fi
    ...
    printf "  %-20s │  " "High"
    if [[ "$high" -gt 0 ]]; then
        printf "${YELLOW}%s files${NC} - Suspicious behaviors\n" "$high"
    else
        printf "${GREEN}0${NC}\n"
    fi
    ...
    printf "  %-20s │  " "Medium"
    if [[ "$medium" -gt 0 ]]; then
        printf "%s files - Unusual patterns\n" "$medium"
    else
        printf "${GREEN}0${NC}\n"
    fi

    printf "  %-20s │  ${DIM}%s files${NC}\n" "Low" "$low"
    ...
    if [[ "$crit_count" -gt 0 ]]; then
        ...
        hr "$BOX_LINE" 68
        echo
        printf "  ${BOLD}${RED}CRITICAL FINDINGS${NC} ${DIM}(potential malicious code)${NC}\n"
        printf "  %s\n" "$(printf '%*s' 64 '' | tr ' ' '─')"

        echo "$json" | jq -r '.findings.critical[] | @json' 2>/dev/null | while read -r finding; do
            local path=$(echo "$finding" | jq -r '.path // "unknown"')
            local behaviors=$(echo "$finding" | jq -r '.behaviors // []')

            printf "\n  ${RED}[CRITICAL]${NC} %s\n" "$path"

            echo "$behaviors" | jq -r '.[] | "\(.description // .id)|\(.matches // [] | join(", "))"' 2>/dev/null | head -3 | while IFS='|' read -r desc matches; do
                printf "    ${DIM}→${NC} %s\n" "$desc"
                if [[ -n "$matches" ]] && [[ "$matches" != "" ]]; then
                    printf "      ${DIM}Evidence: %s${NC}\n" "$(echo "$matches" | cut -c1-60)"
                fi
            done
        done
    fi
    ...
    if [[ "$high_count" -gt 0 ]]; then
        ...
        hr "$BOX_LINE" 68
        echo
        printf "  ${BOLD}${YELLOW}HIGH-RISK FINDINGS${NC} ${DIM}(suspicious behaviors)${NC}\n"
        printf "  %s\n" "$(printf '%*s' 64 '' | tr ' ' '─')"

        # Process first 10 high-risk findings with relative paths
        echo "$json" | jq -r '.findings.high[:10][] |
            (.path | split("/repo/") | if length > 1 then .[1] else . end) as $relpath |
            "\($relpath)|\(.behaviors[:2] | map("\(.description // .id)~\(.matches // [] | .[0] // "")") | join("|"))"' 2>/dev/null | while IFS='|' read -r relpath behavior1 behavior2; do
            printf "\n  ${YELLOW}[high]${NC} %s\n" "$relpath"

            if [[ -n "$behavior1" ]]; then
                local desc1="${behavior1%%~*}"
                local evidence1="${behavior1#*~}"
                printf "    ${DIM}→${NC} %s\n" "$desc1"
                if [[ -n "$evidence1" ]]; then
                    printf "      ${CYAN}%s${NC}\n" "$(echo "$evidence1" | cut -c1-55)"
                fi
            fi
            if [[ -n "$behavior2" ]]; then
                local desc2="${behavior2%%~*}"
                local evidence2="${behavior2#*~}"
                printf "    ${DIM}→${NC} %s\n" "$desc2"
                if [[ -n "$evidence2" ]]; then
                    printf "      ${CYAN}%s${NC}\n" "$(echo "$evidence2" | cut -c1-55)"
                fi
            fi
        done

        if [[ "$high_count" -gt 10 ]]; then
            printf "\n  ${DIM}... and %d more high-risk files${NC}\n" "$((high_count - 10))"
        fi
    fi
    ...
    if [[ "$med_count" -gt 0 ]]; then
        ...
        hr "$BOX_LINE" 68
        echo
        printf "  ${BOLD}MEDIUM-RISK FINDINGS${NC} ${DIM}(showing top 5 of %s)${NC}\n" "$med_count"
        printf "  %s\n" "$(printf '%*s' 64 '' | tr ' ' '─')"

        # Show medium findings with relative paths
        echo "$json" | jq -r '.findings.medium[:5][] |
            (.path | split("/repo/") | if length > 1 then .[1] else . end) as $relpath |
            "\($relpath)|\(.behaviors[0].description // .behaviors[0].id // "suspicious pattern")"' 2>/dev/null | while IFS='|' read -r relpath desc; do
            printf "  ${DIM}[medium]${NC} %s\n" "$relpath"
            printf "    ${DIM}→ %s${NC}\n" "$desc"
        done
    fi
    ...
    hr "$BOX_LINE" 68
    echo
    printf "  ${BOLD}SCOUT RECOMMENDATIONS${NC}\n"
    printf "  %s\n" "$(printf '%*s' 64 '' | tr ' ' '─')"

    if [[ "$critical" -gt 0 ]]; then
        printf "  ${RED}1.${NC} IMMEDIATE: Review critical files for potential backdoors\n"
        printf "  ${RED}2.${NC} Quarantine affected code until investigation complete\n"
        printf "  ${RED}3.${NC} Check git history for unauthorized commits\n"
    elif [[ "$high" -gt 0 ]]; then
        printf "  ${YELLOW}1.${NC} Review high-risk files for intentional vs malicious behavior\n"
        printf "  ${YELLOW}2.${NC} Consider code audit for external network access patterns\n"
        printf "  ${YELLOW}3.${NC} Verify any encoded/obfuscated code sections\n"
    else
        printf "  ${GREEN}1.${NC} No critical/high-risk findings - continue monitoring\n"
        printf "  ${GREEN}2.${NC} Review medium-risk items during regular security reviews\n"
    fi

    echo
    hr "$BOX_LINE" 68
    echo -e "  ${DIM}Scout Agent • Phantom Supply Chain Security • v${REPORT_VERSION}${NC}"
    echo
}

# Format org malcontent report for terminal
format_org_malcontent_terminal() {
    local json="$1"
    ...
    local repos_with_issues=$(echo "$json" | jq -r '.projects.with_issues | join(", ")' 2>/dev/null)

    echo
    printf "${BOLD}${BOX_TL}"
    printf '%*s' 66 '' | tr ' ' "$BOX_H"
    printf "${BOX_TR}${NC}\n"
    printf "${BOLD}${BOX_V}  SCOUT: ORGANIZATION SECURITY REPORT%*s${BOX_V}${NC}\n" 28 ''
    printf "${BOLD}${BOX_BL}"
    printf '%*s' 66 '' | tr ' ' "$BOX_H"
    printf "${BOX_BR}${NC}\n"
    echo

    printf "  ${BOLD}Organization:${NC}  %s\n" "$org"
    printf "  ${BOLD}Repositories:${NC}  %s\n" "$project_count"
    printf "  ${BOLD}Files Scanned:${NC} %s\n" "$(format_number "$total_files")"

    echo
    hr "$BOX_LINE" 68
    echo

    printf "  ${BOLD}AGGREGATE FINDINGS${NC}\n"
    printf "  %s\n" "$(printf '%*s' 64 '' | tr ' ' '─')"
    ...
    if [[ -n "$repos_with_issues" ]] && [[ "$repos_with_issues" != "" ]]; then
        echo
        hr "$BOX_LINE" 68
        echo
        printf "  ${BOLD}REPOSITORIES WITH ISSUES${NC}\n"
        printf "  %s\n" "$(printf '%*s' 64 '' | tr ' ' '─')"
        printf "  ${YELLOW}%s${NC}\n" "$repos_with_issues"
    fi
...
export -f format_malcontent_terminal
export -f format_project_malcontent_terminal
export -f format_org_malcontent_terminal
